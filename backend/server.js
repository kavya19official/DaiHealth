...
// ============================================================================
// DAI Hospital-side demo API
// Deterministic continuity, registry, clinic-flow, draft-review, and audit
// endpoints. This demo surface intentionally avoids diagnosis, treatment
// recommendation, clinical interpretation, or clinical risk scoring.
// ============================================================================
const HOSPITAL_TODAY = '2026-09-20T09:00:00+05:30';
const DAY_MS_HOSPITAL = 24 * 60 * 60 * 1000;

const hospitalPathways = {
  maternal: {
...
const hospitalPatients = [
  { id: 'P-1001', name: 'Asha Nair', age: 31, program: 'maternal', provider: 'Dr. S. Kumar', stage: 'Trimester 2', pathwayStart: '2026-04-07', lastTouch: '2026-08-28', noShows: 1, phone: '+91 90000 11111' },
  { id: 'P-1002', name: 'Meera Joshi', age: 28, program: 'maternal', provider: 'Dr. S. Kumar', stage: 'Trimester 3', pathwayStart: '2025-12-10', lastTouch: '2026-08-02', noShows: 3, phone: '+91 90000 22222' },
  { id: 'P-2030', name: 'Farhan Ali', age: 54, program: 'diabetes', provider: 'Dr. N. Rao', stage: 'Quarterly labs', pathwayStart: '2026-02-08', lastTouch: '2026-08-14', noShows: 2, phone: '+91 90000 33333' },
  { id: 'P-3091', name: 'Leela Menon', age: 47, program: 'oncology', provider: 'Dr. A. Shah', stage: 'Follow-up imaging', pathwayStart: '2026-05-14', lastTouch: '2026-09-17', noShows: 0, phone: '+91 90000 44444' },
  { id: 'P-3094', name: 'Ritika Sen', age: 39, program: 'oncology', provider: 'Dr. A. Shah', stage: 'Investigation', pathwayStart: '2026-06-01', lastTouch: '2026-07-24', noShows: 2, phone: '+91 90000 55555' }
];

const hospitalCareEvents = [
...
const hospitalState = {
  ...
  consultationBrief: {
    ...
    facts: [
      { label: 'Last ANC visit', value: '2026-03-10', source: 'EMR visit-4204', confidence: 'typed/coded' },
      { label: 'Anomaly scan document', value: 'Uploaded 2026-05-01', source: 'Radiology scan-8322', confidence: 'typed/coded' },
      { label: 'Referral letter', value: 'Mentions scan planned after 2026-08-15', source: 'OCR doc-ref-192', confidence: '0.82' }
    ],
    flags: [
  ...
  queue: [
    { slot: '09:30', patientId: 'P-1002', status: 'checked in', waitMins: 18, urgency: 'Staff-set: high-priority follow-up' },
    { slot: '09:45', patientId: 'P-3091', status: 'waiting', waitMins: 11, urgency: '' },
    { slot: '10:00', patientId: 'P-2030', status: 'delayed labs', waitMins: 8, urgency: '' },
    { slot: '10:15', patientId: 'P-1001', status: 'cancelled', waitMins: 0, urgency: '' }
  ],
  drafts: [
    { id: 'D-501', type: 'referral letter', patientId: 'P-1002', status: 'pending_review', body: 'Draft referral letter assembled from reviewed demographics, recent appointment history, and listed document references. Clinician must edit and approve before filing.' },
    { id: 'D-502', type: 'patient handout', patientId: null, status: 'draft', body: 'Draft multilingual clinic handout based on the hospital-approved antenatal visit schedule knowledge base.' },
    { id: 'D-503', type: 'department report', patientId: null, status: 'approved', body: 'Operational report draft: open follow-up gaps by program, outreach workload, and slot utilization. No clinical inference included.' }
  ],
  audit: []
};

function hospitalDaysBetween(a, b) {
  return Math.floor((new Date(a).setHours(0, 0, 0, 0) - new Date(b).setHours(0, 0, 0, 0)) / DAY_MS_HOSPITAL);
}

function hospitalAddDays(value, days) {
  return new Date(new Date(value).getTime() + days * DAY_MS_HOSPITAL);
}

function hospitalAudit(actor, action, entityType, entityId, detail) {
  const entry = {
    id: `AUD-${Math.random().toString(16).slice(2, 8).toUpperCase()}`,
    actor: actor || 'hospital-user',
    action,
    entityType,
    entityId,
    detail,
  ...
  hospitalState.audit.unshift(entry);
  return entry;
}
...
function computeHospitalGaps() {
  ...
  hospitalPatients.forEach((patient) => {
    const template = hospitalPathways[patient.program];
    template.events.forEach((expected) => {
      const dueDate = hospitalAddDays(patient.pathwayStart, expected.offset);
      const daysOverdue = hospitalDaysBetween(today, dueDate);
      const actual = hospitalCareEvents.find((event) =>
        event.patientId === patient.id &&
        event.type === expected.type &&
        new Date(event.recordedAt) <= today
      );
      const id = `${patient.id}:${expected.id}`;
      if (!actual && daysOverdue > 0 && !hospitalState.resolvedGapIds[id]) {
        gaps.push({
          id,
          ...
          assignedTo: patient.provider,
          operationalPriority: daysOverdue + patient.noShows * 7,
          reviewStatus: 'open',
          provenance: `Expected event ${expected.id} from ${template.name}; no matching recorded care event found in EMR/lab/document sources.`
        });
      }
    });
  });
...
function hospitalSnapshot() {
  return {
    ...
    gaps: computeHospitalGaps(),
    audit: hospitalState.audit
  };
}

hospitalAudit('system', 'gap_engine_run', 'CareGap', 'all', 'Deterministic expected-versus-recorded diff completed.');
hospitalAudit('Dr. S. Kumar', 'record_accessed', 'Patient', 'P-1002', 'Opened hospital registry detail panel.');

app.get('/api/hospital/state', (req, res) => {
  res.json(hospitalSnapshot());
});

app.get('/api/hospital/patients/:id/gaps', (req, res) => {
  const gaps = computeHospitalGaps().filter((gap) => gap.patientId === req.params.id);
  res.json({ gaps, reviewStatus: 'human_review_required' });
});

app.get('/api/hospital/gaps', (req, res) => {
  const { program, provider, overdue_gt } = req.query;
  const overdueGt = overdue_gt ? Number(overdue_gt) : 0;
  const gaps = computeHospitalGaps().filter((gap) => {
    if (program && gap.program !== program) return false;
    if (provider && gap.provider !== provider) return false;
    if (overdueGt && gap.daysOverdue <= overdueGt) return false;
    return true;
  });
  res.json({ gaps, scoringLabel: 'Operational outreach priority, not clinical risk.' });
});

app.post('/api/hospital/gaps/:id/contact', (req, res) => {
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'contact', 'CareGap', req.params.id, req.body.notes || 'Contact patient confirmed by user.');
  res.json({ message: 'Contact action audited', state: hospitalSnapshot() });
});

app.post('/api/hospital/gaps/:id/reassign', (req, res) => {
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'reassign', 'CareGap', req.params.id, req.body.notes || 'Reassignment confirmed by user.');
  res.json({ message: 'Reassignment audited', state: hospitalSnapshot() });
});

app.post('/api/hospital/gaps/:id/resolve', (req, res) => {
  hospitalState.resolvedGapIds[req.params.id] = true;
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'resolve', 'CareGap', req.params.id, req.body.notes || 'Gap marked resolved by user.');
  res.json({ message: 'Gap resolved and audited', state: hospitalSnapshot() });
});

app.post('/api/hospital/clinic/slots/:id/reassign', (req, res) => {
  hospitalAudit(req.body.actor || 'front-desk', 'approve-slot', 'CareGap', req.params.id, req.body.notes || 'Front desk approved a slot suggestion.');
  res.json({ message: 'Slot suggestion approval audited', state: hospitalSnapshot() });
});

app.get('/api/hospital/consultations/:id/brief', (req, res) => {
  res.json({ brief: hospitalState.consultationBrief, reviewStatus: hospitalState.consultationBrief.status });
});

app.post('/api/hospital/consultations/:id/brief/review', (req, res) => {
  hospitalState.consultationBrief.status = 'reviewed';
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'brief_reviewed', 'ConsultationBrief', req.params.id, req.body.notes || 'Fact-only brief reviewed by clinician.');
  res.json({ message: 'Brief reviewed', state: hospitalSnapshot() });
});

app.post('/api/hospital/consultations/:id/brief/reject', (req, res) => {
  hospitalState.consultationBrief.status = 'rejected_item';
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'brief_item_rejected', 'ConsultationBrief', req.params.id, req.body.notes || 'Clinician rejected an extracted item for correction.');
  res.json({ message: 'Brief item rejected', state: hospitalSnapshot() });
});

app.post('/api/hospital/content/draft', (req, res) => {
  const id = `D-${Math.floor(600 + Math.random() * 300)}`;
  const draft = {
    id,
    type: req.body.type || 'patient handout',
    patientId: req.body.patientId || null,
    status: 'draft',
    body: req.body.body || 'New draft created from approved hospital source material. Review and edit required before publication.'
  };
  hospitalState.drafts.unshift(draft);
  hospitalAudit('drafting-agent', 'draft_generated', 'ContentDraft', id, 'Draft generated; no send/file action performed.');
  res.status(201).json({ draft, state: hospitalSnapshot() });
});

app.post('/api/hospital/content/:id/approve', (req, res) => {
  const draft = hospitalState.drafts.find((item) => item.id === req.params.id);
  if (!draft) return res.status(404).json({ error: 'Draft not found' });
  draft.status = 'approved';
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'approve-draft', 'ContentDraft', draft.id, req.body.notes || 'Draft approved by clinician.');
  res.json({ message: 'Draft approved', state: hospitalSnapshot() });
});

app.post('/api/hospital/content/:id/reject', (req, res) => {
  const draft = hospitalState.drafts.find((item) => item.id === req.params.id);
  if (!draft) return res.status(404).json({ error: 'Draft not found' });
  draft.status = 'rejected';
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'reject-draft', 'ContentDraft', draft.id, req.body.notes || 'Draft rejected by clinician.');
  res.json({ message: 'Draft rejected', state: hospitalSnapshot() });
});

app.post('/api/hospital/content/:id/publish', (req, res) => {
  const draft = hospitalState.drafts.find((item) => item.id === req.params.id);
  if (!draft) return res.status(404).json({ error: 'Draft not found' });
  if (draft.status !== 'approved') {
    return res.status(409).json({ error: 'Publishing is blocked until a clinician approves the draft.' });
  }
  draft.status = 'published';
  hospitalAudit(req.body.actor || 'Dr. S. Kumar', 'publish-draft', 'ContentDraft', draft.id, req.body.notes || 'Approved draft published/filed by clinician.');
  res.json({ message: 'Draft published/filed', state: hospitalSnapshot() });
});
