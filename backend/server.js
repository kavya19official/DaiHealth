...
const COMPANION_MODEL = process.env.DAI_COMPANION_MODEL || process.env.OPENAI_MODEL || 'gpt-4o-mini';
const COMPANION_API_KEY = process.env.DAI_COMPANION_API_KEY || process.env.OPENAI_API_KEY || '';

function companionFallback(question, role) {
  const q = String(question || '').toLowerCase();
  const safePrefix = role === 'doctor'
    ? 'For the doctor workflow: '
    : role === 'hospital'
      ? 'For the hospital team workflow: '
      : '';

  if (/(dangerous|danger|symptom|bleeding|pain|fever|breath|dizzy|faint|swelling|headache|what.?s wrong|what is wrong|wrong with me|emergency)/i.test(q)) {
    return 'I cannot tell whether a symptom is dangerous or diagnose what is happening. Please contact your doctor or hospital now. If this feels urgent, use your local emergency number or the SOS option so a human care team can respond.';
  }
  if (/(should i take|can i take|medicine|medication|tablet|dose|dosage|antibiotic|painkiller|supplement)/i.test(q)) {
    return 'I cannot advise whether to take a medicine, supplement, or dose. Please ask your doctor, pharmacist, or care team. I can help you list the question clearly before you message them.';
  }
  if (/(am i high.?risk|is my pregnancy high.?risk|high-risk|high risk)/i.test(q)) {
    return 'Only a qualified clinician can decide whether a pregnancy is high-risk. दाई can show your recorded appointments, tests, and flags, but it does not make clinical risk decisions.';
  }
  if (/(test|scan|this month|20.?week|twenty.?week|bring|appointment|checkup|check-up)/i.test(q)) {
    return safePrefix + 'Check your care timeline and upcoming appointment card first. It should show expected tests, scans, and documents to bring. If something was missed, contact the clinic so staff can reschedule or mark the gap for follow-up.';
  }
  if (/(vaccination|vaccine|baby.*next|immuni[sz]ation)/i.test(q)) {
    return 'Your baby’s next vaccination should appear in the child timeline with its due date. If it is overdue or missing, contact your clinic or PHC so the schedule can be confirmed by a human care worker.';
  }
  if (/(gestational diabetes|anemia|anaemia|term|means|why do i need)/i.test(q)) {
    return 'I can explain terms in plain language, but I cannot interpret your personal reports. Gestational diabetes means high blood sugar first recognized during pregnancy. Anaemia means low haemoglobin. Your doctor should explain what your own values mean for you.';
  }
  if (/(nearest clinic|reschedule|documents|government scheme|scheme|where|how do i)/i.test(q)) {
    return 'For logistics, use the appointments, documents, and messages sections. You can request a reschedule, check uploaded documents, and ask your care team which scheme documents are needed for your facility.';
  }
  if (/(nervous|scared|anxious|worried|forgot)/i.test(q)) {
    return 'It is understandable to feel worried. I can help you prepare questions for your doctor and remind you of the next step, but I cannot judge symptoms or medication safety. If you feel unwell or unsafe, contact your care team.';
  }
  return safePrefix + 'I can help with care timelines, appointment logistics, document reminders, vaccination milestones, and how to use दाई. I cannot diagnose, recommend treatment, score clinical risk, or replace your doctor.';
}

function shouldForceSafeFallback(question) {
  const q = String(question || '').toLowerCase();
  return /(dangerous|symptom|bleeding|severe|pain|fever|breath|dizzy|faint|what.?s wrong|what is wrong|should i take|can i take|medication|medicine|dose|dosage|am i high.?risk|is my pregnancy high.?risk)/i.test(q);
}

app.post('/api/companion/chat', async (req, res) => {
  try {
    const question = String((req.body && req.body.message) || '').trim().slice(0, 800);
    const role = String((req.body && req.body.role) || 'mother').replace(/[^a-z-]/gi, '').slice(0, 24) || 'mother';
    if (!question) return res.status(400).json({ error: 'Message is required' });

    if (shouldForceSafeFallback(question) || !COMPANION_API_KEY) {
      return res.json({
        answer: companionFallback(question, role),
        source: COMPANION_API_KEY ? 'safety-fallback' : 'fallback',
        safety: 'assistive_only'
      });
    }
        ...
        messages: [
          {
            role: 'system',
            content: [
              'You are दाई AI Care Companion for a healthcare workflow demo.',
              'You are assistive, not clinical. Never diagnose, recommend treatment, score clinical risk, interpret personal reports, or say a symptom is safe or dangerous.',
              'Answer only general education, milestones, logistics, document reminders, appointment preparation, and product navigation.',
              'For symptoms, medicines, high-risk pregnancy, or "what is wrong with me", refuse briefly and redirect to the doctor, hospital, emergency number, or SOS workflow.',
              'Use calm, plain English. Keep answers under 90 words.'
            ].join(' ')
          },
          { role: 'user', content: `Portal role: ${role}\nQuestion: ${question}` }
        ]
      })
    });

    if (!response.ok) {
      return res.json({ answer: companionFallback(question, role), source: 'fallback', safety: 'assistive_only' });
    }
    const data = await response.json();
    const answer = data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content;
    res.json({
      answer: String(answer || companionFallback(question, role)).trim(),
      source: 'ai',
      safety: 'assistive_only'
    });
  } catch (error) {
    console.error('Companion chat error:', error);
    res.json({ answer: companionFallback(req.body && req.body.message, req.body && req.body.role), source: 'fallback', safety: 'assistive_only' });
  }
});
