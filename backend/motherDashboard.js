...
//   GET    /api/payment-methods           the mother's saved payment methods
//   POST   /api/payment-methods
//   PATCH  /api/payment-methods/:id       set as primary
//   DELETE /api/payment-methods/:id       (soft delete)
//   GET    /api/profile/financial         payment methods + self-reported stability
//   PATCH  /api/profile/financial         set financial_stability
// ============================================================================
...
const PAYMENT_METHOD_TYPES = ['insurance', 'mobile_money', 'bank_card', 'cash', 'employer_scheme'];
const FINANCIAL_STABILITY_VALUES = ['comfortable', 'manageable', 'struggling'];
...
module.exports = function registerMotherDashboardRoutes(app, { pool, authenticateToken, authorizeRole, MILESTONE_TEMPLATE }) {
  ...
  // ==========================================================================
  // Payment methods + self-reported financial stability
  // No card/account numbers are accepted — "reference" is a short, mother-
  // chosen label (e.g. last 4 digits, a policy nickname), never validated as
  // or stored as a real account number.
  // ==========================================================================
  app.get('/api/payment-methods', ...motherOnly, async (req, res) => {
    try {
      const r = await pool.query(
        `SELECT id, method_type, provider, reference, is_primary
           FROM payment_methods WHERE mother_id = $1 AND active
           ORDER BY is_primary DESC, created_at, id`,
        [req.user.id]
      );
      res.json({ payment_methods: r.rows });
    } catch (e) { fail(res, 'Fetch payment methods', e, 'Failed to load payment methods'); }
  });

  app.post('/api/payment-methods', ...motherOnly, async (req, res) => {
    const client = await pool.connect();
    try {
      const b = req.body || {};
      if (!PAYMENT_METHOD_TYPES.includes(b.method_type)) {
        return res.status(400).json({ error: 'Choose insurance, mobile money, bank card, cash, or an employer scheme' });
      }
      const provider = cleanStr(b.provider, 120) || null;
      const reference = cleanStr(b.reference, 50) || null;
      const makePrimary = !!b.is_primary;

      await client.query('BEGIN');
      const existing = await client.query(
        'SELECT COUNT(*)::int AS n FROM payment_methods WHERE mother_id = $1 AND active', [req.user.id]
      );
      if (existing.rows[0].n >= 10) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: 'You can track up to 10 payment methods' });
      }
      // First payment method is always primary, regardless of what was sent.
      const isFirst = existing.rows[0].n === 0;
      if (makePrimary || isFirst) {
        await client.query('UPDATE payment_methods SET is_primary = FALSE WHERE mother_id = $1 AND active', [req.user.id]);
      }
      const r = await client.query(
        `INSERT INTO payment_methods (mother_id, method_type, provider, reference, is_primary)
         VALUES ($1, $2, $3, $4, $5) RETURNING id, method_type, provider, reference, is_primary`,
        [req.user.id, b.method_type, provider, reference, makePrimary || isFirst]
      );
      await client.query('COMMIT');
      res.status(201).json({ message: 'Payment method added', payment_method: r.rows[0] });
    } catch (e) {
      await client.query('ROLLBACK').catch(() => {});
      fail(res, 'Add payment method', e, 'Failed to add payment method');
    } finally {
      client.release();
    }
  });

  // Only field this supports right now is making a method primary.
  app.patch('/api/payment-methods/:id', ...motherOnly, async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (Number.isNaN(id)) return res.status(400).json({ error: 'Invalid id' });
    if (!(req.body || {}).is_primary) return res.status(400).json({ error: 'Nothing to update' });

    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      const own = await client.query(
        'SELECT id FROM payment_methods WHERE id = $1 AND mother_id = $2 AND active', [id, req.user.id]
      );
      if (!own.rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Payment method not found' }); }
      await client.query('UPDATE payment_methods SET is_primary = FALSE WHERE mother_id = $1 AND active', [req.user.id]);
      const r = await client.query(
        'UPDATE payment_methods SET is_primary = TRUE WHERE id = $1 RETURNING id, method_type, provider, reference, is_primary',
        [id]
      );
      await client.query('COMMIT');
      res.json({ message: 'Primary payment method updated', payment_method: r.rows[0] });
    } catch (e) {
      await client.query('ROLLBACK').catch(() => {});
      fail(res, 'Update payment method', e, 'Failed to update payment method');
    } finally {
      client.release();
    }
  });

  app.delete('/api/payment-methods/:id', ...motherOnly, async (req, res) => {
    try {
      const id = parseInt(req.params.id, 10);
      if (Number.isNaN(id)) return res.status(400).json({ error: 'Invalid id' });
      const r = await pool.query(
        'UPDATE payment_methods SET active = FALSE, is_primary = FALSE WHERE id = $1 AND mother_id = $2 AND active RETURNING id',
        [id, req.user.id]
      );
      if (!r.rows.length) return res.status(404).json({ error: 'Payment method not found' });
      res.json({ message: 'Payment method removed' });
    } catch (e) { fail(res, 'Remove payment method', e, 'Failed to remove payment method'); }
  });

  app.get('/api/profile/financial', ...motherOnly, async (req, res) => {
    try {
      const [stability, methods] = await Promise.all([
        pool.query('SELECT financial_stability FROM mother_profiles WHERE user_id = $1', [req.user.id]),
        pool.query(
          `SELECT id, method_type, provider, reference, is_primary
             FROM payment_methods WHERE mother_id = $1 AND active
             ORDER BY is_primary DESC, created_at, id`,
      ...
      if (!stability.rows.length) return res.status(404).json({ error: 'Profile not found' });
      res.json({
        financial_stability: stability.rows[0].financial_stability,
        payment_methods: methods.rows,
      });
    } catch (e) { fail(res, 'Fetch financial profile', e, 'Failed to load financial details'); }
  });

  app.patch('/api/profile/financial', ...motherOnly, async (req, res) => {
    const { financial_stability } = req.body || {};
    if (!FINANCIAL_STABILITY_VALUES.includes(financial_stability)) {
      return res.status(400).json({ error: 'Choose comfortable, manageable, or struggling' });
    }
    try {
      const r = await pool.query(
        'UPDATE mother_profiles SET financial_stability = $2 WHERE user_id = $1 RETURNING financial_stability',
        [req.user.id, financial_stability]
      );
      if (!r.rows.length) return res.status(404).json({ error: 'Profile not found' });
      res.json({ message: 'Saved', financial_stability: r.rows[0].financial_stability });
    } catch (e) { fail(res, 'Update financial profile', e, 'Failed to save'); }
  });
};
